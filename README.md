...
- 프론트엔드: `cd frontend && npm ci --include=dev` (devDependencies까지 설치해야 Vite/TS 빌드 오류가 없습니다)
...
- 프론트 빌드 결과를 사용해 단일 앱으로 실행하려면 `npm run build` 후 uvicorn만 실행하면 됩니다. (빌드 산출물이 없으면 정적 서빙이 비활성화되고 API만 동작하며 503을 반환합니다.)
...
6. 배포: `fly deploy` (Dockerfile에서 `npm ci --include=dev`로 devDependencies까지 설치하고, `uvicorn[standard]`로 웹소켓 안정성을 확보합니다)
